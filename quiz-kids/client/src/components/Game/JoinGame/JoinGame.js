    ...
    <div className={styles.page}>
      {!isPlayerAdded ? (
        <div className={styles.section}>
          <h2>{isLanguageEnglish ? "Join game" : "Tham gia"}</h2>
          <input
            type="text"
            ref={pinRef}
            placeholder={
              isLanguageEnglish ? "Write here a pin" : "Nhập mã pin"
            }
          />
        ...
        <div className={styles.section}>
          <h2>
            {isLanguageEnglish ? "You joined the game" : "Bạn đã tham gia"}
          </h2>
          <h4>
            {isLanguageEnglish ? "Waiting on a host to start the game" : "Chờ người chủ trì bắt đầu"}
          </h4>
          <CircularProgress />
        </div>
      )}
